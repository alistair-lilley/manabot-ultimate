...
class TGInterface(Dispatcher, BaseBot):
    ...
    async def on_inline(self, inline_query: types.InlineQuery):
        ...
        full_card = self._database.retrieve_card(card, Platform.TELEGRAM)
        image = types.InputTextMessageContent(
            markdown.link(".", full_card.image), parse_mode="MarkdownV2"
        )
        text = types.InputTextMessageContent(full_card.text, parse_mode="MarkdownV2")
        name = full_card.text.split("\n")[0]
        result_id: str = hashlib.md5(card.encode()).hexdigest()
        image_item = types.InlineQueryResultArticle(
            id=result_id, title=f"Link to {name}", input_message_content=image
        )
        text_item = types.InlineQueryResultArticle(
            id=result_id + "1", title=f"Text of {name}", input_message_content=text
        )
        await self._bot.answer_inline_query(
            inline_query.id, results=[image_item, text_item], cache_time=1
        )
