...
class CardDatabase:
    ...
    def get_card(
        ...
        return self._retrieve(matched_card, tgdc, find_other_faces)
    ...
    def _retrieve(
        self: CardDatabase, cardname: str, tgdc: Platform, find_other_faces: bool
    ) -> List[CardResult]:
        results = list()
        card = self._cards[cardname]
        if card.other_faces_list and find_other_faces:
            for face in card.other_faces_list:
                results += self.get_card(face, tgdc, find_other_faces=False)
